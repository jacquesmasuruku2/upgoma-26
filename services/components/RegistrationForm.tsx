...
const RegistrationForm: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isSupabaseConfigured()) {
      setErrorFeedback("Supabase n'est pas configuré. Vérifiez votre configuration.");
      return;
    }
    ...
    try {
      ...
      const BUCKET_NAME = 'form_site_files';

      // 1. Upload de la Photo dans le dossier 'photos/' uniquement
      if (formData.passportPhoto) {
        const ext = formData.passportPhoto.name.split('.').pop()?.toLowerCase();
        const filePath = `photos/photo-${timestamp}-${Math.random().toString(36).substring(7)}.${ext}`;
        
        const { data, error: photoError } = await supabase.storage
          .from(BUCKET_NAME)
          .upload(filePath, formData.passportPhoto);
        
        if (photoError) throw new Error(`Erreur Stockage Photo: ${photoError.message}`);
        passportPhotoUrl = data.path;
      }

      // 2. Upload du Document Académique (PDF) dans le dossier 'documents/' uniquement
      if (formData.documents && formData.documents.length > 0) {
        const docFile = formData.documents[0];
        const ext = docFile.name.split('.').pop()?.toLowerCase();
        const filePath = `documents/docs-${timestamp}-${Math.random().toString(36).substring(7)}.${ext}`;
        
        const { data, error: docError } = await supabase.storage
          .from(BUCKET_NAME)
          .upload(filePath, docFile);
        
        if (docError) throw new Error(`Erreur Stockage Document: ${docError.message}`);
      ...
      }).catch(err => console.warn("L'email de confirmation n'a pas pu être envoyé.", err));

      setStep(FormStep.CONFIRMATION);
    } catch (error: any) {
      console.error("Erreur critique d'inscription:", error);
      setErrorFeedback(error.message || "Une erreur est survenue lors de la soumission.");
    } finally {
    ...
    return (
      <div className="bg-white dark:bg-slate-900 p-12 rounded-3xl shadow-xl text-center max-w-2xl mx-auto border-t-8 border-upgGold transition-colors">
        ...
        <p className="text-slate-600 dark:text-slate-400 mb-8 leading-relaxed">
          Félicitations <strong>{formData.firstName} {formData.lastName}</strong>. Votre dossier a été enregistré avec succès. <br/><span className="text-upgGold font-semibold">Un email de confirmation vient de vous être envoyé.</span>
        </p>
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl overflow-hidden max-w-5xl mx-auto flex flex-col md:flex-row min-h-[700px] transition-colors border border-slate-100 dark:border-slate-800">
      ...
      <form onSubmit={step === FormStep.DOCUMENTS ? handleSubmit : handleNext} className="p-8 md:p-12 flex-1 space-y-6">
        {errorFeedback && (
          <div className="bg-red-50 dark:bg-red-900/20 border-l-4 border-red-500 p-4 mb-4 flex items-center gap-3">
          ...
          <div className="space-y-6 animate-in slide-in-from-right-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="space-y-1">
                ...
                <input required name="firstName" value={formData.firstName} onChange={handleChange} className="w-full bg-slate-50 dark:bg-slate-800 p-3 rounded-xl outline-none focus:ring-2 focus:ring-upgGold dark:text-white border border-slate-200 dark:border-slate-700" placeholder="Jacques" />
              </div>
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Nom</label>
                <input required name="lastName" value={formData.lastName} onChange={handleChange} className="w-full bg-slate-50 dark:bg-slate-800 p-3 rounded-xl outline-none focus:ring-2 focus:ring-upgGold dark:text-white border border-slate-200 dark:border-slate-700" placeholder="MAPENZI" />
              </div>
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Post-nom</label>
                <input name="middleName" value={formData.middleName} onChange={handleChange} className="w-full bg-slate-50 dark:bg-slate-800 p-3 rounded-xl outline-none focus:ring-2 focus:ring-upgGold dark:text-white border border-slate-200 dark:border-slate-700" placeholder="MASURUKU" />
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
              <div className="space-y-1">
                ...
                <select name="gender" value={formData.gender} onChange={handleChange} className="w-full bg-slate-50 dark:bg-slate-800 p-3 rounded-xl outline-none dark:text-white border border-slate-200 dark:border-slate-700">
                  <option value="M">Masculin</option>
                  <option value="F">Féminin</option>
                </select>
              </div>
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">État Civil</label>
                <select name="maritalStatus" value={formData.maritalStatus} onChange={handleChange} className="w-full bg-slate-50 dark:bg-slate-800 p-3 rounded-xl outline-none dark:text-white border border-slate-200 dark:border-slate-700">
              ...
              <div className="space-y-1">
                ...
                <input required type="date" name="birthDate" value={formData.birthDate || ''} onChange={handleChange} className="w-full bg-slate-50 dark:bg-slate-800 p-3 rounded-xl outline-none dark:text-white border border-slate-200 dark:border-slate-700" />
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="space-y-1">
                ...
                <input required name="birthPlace" value={formData.birthPlace || ''} onChange={handleChange} className="w-full bg-slate-50 dark:bg-slate-800 p-3 rounded-xl outline-none dark:text-white border border-slate-200 dark:border-slate-700" placeholder="Goma" />
              </div>
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Téléphone</label>
                <input required type="tel" name="phone" value={formData.phone || ''} onChange={handleChange} className="w-full bg-slate-50 dark:bg-slate-800 p-3 rounded-xl outline-none dark:text-white border border-slate-200 dark:border-slate-700" placeholder="+243..." />
              </div>
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Email</label>
                <input required type="email" name="email" value={formData.email || ''} onChange={handleChange} className="w-full bg-slate-50 dark:bg-slate-800 p-3 rounded-xl outline-none dark:text-white border border-slate-200 dark:border-slate-700" placeholder="etudiant@mail.com" />
              </div>
            </div>
        ...
        {step === FormStep.ACADEMIC && (
          <div className="space-y-6 animate-in slide-in-from-right-4">
            ...
            <div className="space-y-1">
              <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">École secondaire d'origine</label>
              <input required name="previousSchool" value={formData.previousSchool || ''} onChange={handleChange} className="w-full bg-slate-50 dark:bg-slate-800 p-3 rounded-xl outline-none dark:text-white border border-slate-200 dark:border-slate-700" placeholder="Nom de l'école" />
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Faculté</label>
                <select required name="targetFaculty" value={formData.targetFaculty || ''} onChange={handleChange} className="w-full bg-slate-50 dark:bg-slate-800 p-4 rounded-xl outline-none dark:text-white border border-slate-200 dark:border-slate-700">
              ...
              <div className="space-y-1">
                ...
                <select required name="targetDepartment" value={formData.targetDepartment || ''} onChange={handleChange} disabled={!formData.targetFaculty} className="w-full bg-slate-50 dark:bg-slate-800 p-4 rounded-xl outline-none dark:text-white opacity-100 disabled:opacity-50 border border-slate-200 dark:border-slate-700">
        ...
        {step === FormStep.DOCUMENTS && (
          <div className="space-y-6 animate-in slide-in-from-right-4">
                ...
                <label htmlFor="pdf-upload" className="cursor-pointer flex flex-col items-center group">
                  {hasDocs ? (
                    <>
                      ...
                      <span className="text-sm font-bold text-green-700 dark:text-green-400">Dossier PDF prêt</span>
            ...
            <div className="flex justify-between pt-10">
              ...
              >
                {isSubmitting ? <Loader2 className="animate-spin" size={20} /> : "Finaliser l'Inscription"}
              </button>
            </div>
          </div>
        )}
